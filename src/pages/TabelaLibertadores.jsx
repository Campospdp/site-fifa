      ...
      <div className="flex-grow overflow-y-auto custom-scroll px-1 md:px-6 py-4">
        <div className="max-w-[950px] mx-auto space-y-10">
          ...
          <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {grupos.map((grupo) => (
              <div key={grupo.nome} className="flex flex-col shadow-lg rounded-sm overflow-hidden">
                ...
                {/* Cabeçalho Colunas */}
                <div className={`grid ${gridLayout} gap-0.5 md:gap-2 px-1 md:px-3 py-2 text-[7px] md:text-[10px] bg-[#d9d9d9]/50 border-b border-zinc-800/10 text-zinc-500 uppercase`}>
                ...
                {/* Linhas dos Times */}
                {grupo.times.map((t, i) => (
                  <div key={t.nome} className={`grid ${gridLayout} gap-0.5 md:gap-2 px-1 md:px-3 py-2.5 md:py-3 items-center bg-[#d9d9d9]/70 mt-[1px] border-l-4 ${i < 2 ? 'border-yellow-500' : 'border-transparent'}`}>
                    <span className={`w-4 h-4 md:w-6 md:h-6 flex items-center justify-center rounded-full text-[9px] md:text-[11px] text-white ${i < 2 ? 'bg-[#1a2024]' : 'bg-zinc-500'}`}>
                      {t.pos}
                    </span>
